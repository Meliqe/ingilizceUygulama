...
const Quiz = ({ navigation }) => {
    ...
    const fetchQuestions = useCallback(async () => {
        ...
        setError('');
        try {
            const words = await db.getAllAsync('SELECT * FROM words ORDER BY RANDOM() LIMIT 15');
            if (words.length < 15) {
                setError('Veritabanında yeterli kelime bulunmuyor.');
                return;
            }

            const questions = await Promise.all(words.map(async (word) => {
                const sameWordMeanings = await db.getAllAsync('SELECT meaning FROM words WHERE english_word = ?', [word.english_word]);
                let options = [word.meaning];

                while (options.length < 4) {
                    const randomWord = words[Math.floor(Math.random() * words.length)];
                    if (!sameWordMeanings.some(m => m.meaning === randomWord.meaning) && !options.includes(randomWord.meaning)) {
    ...
    const renderQuestion = () => {
        ...
        return (
            <View style={styles.questionContainer}>
                <Text style={styles.questionText}>"{currentQuestion.word.english_word}" kelimesinin anlamı nedir?</Text>
                        ...
                        renderItem={({ item }) => (
                            <View style={styles.incorrectItem}>
                                <Text style={styles.incorrectQuestionText}>
                                    "{item.question.word.english_word}" kelimesinin anlamı nedir?
                                </Text>
